...
Superset 6.1.0 по умолчанию может применять к Metric адаптивное отображение чисел. Поэтому то же значение на Chart может визуально выглядеть, например, так:
...
Финальный урок проверяет `Table`, `Big Number`, `Bar Chart`, `Line Chart`, обычный Explore Filter, временную группировку и selective Scoping Native Filters. Если весь набор выполняется самостоятельно и контрольные значения совпадают — базовый навык работы с Superset получен.
