...
Команда ниже удаляет volumes текущего Compose-проекта, включая metadata Superset и учебную PostgreSQL. Начинайте из корневого каталога репозитория и переходите в `training` только этой командой:
...
## Итоговая шпаргалка курса

Эта глава по-прежнему **не вводит новых функций**.

После практики 02–11 можно ещё раз коротко проверить архитектурную границу продукта:

→ [Где заканчивается Superset](12a-what-belongs-in-superset.md)

Это сокращённое итоговое возвращение к теме из урока 01, а не новый обязательный блок.
