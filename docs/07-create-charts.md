...
В диалоге сохранения выберите сохранение как нового Chart (`Save as`) и укажите имя:
...
Нажмите `Run Query` и снова получите:
...
На этот раз **не создавайте новый Chart**.

Сохраните изменения поверх существующего Chart (`Overwrite` / сохранение текущего Chart — в зависимости от подписи диалога).
...
Сохраняйте изменения поверх существующего объекта.
...
Вероятно, при сохранении существующего Chart был выбран `Save as` вместо перезаписи текущего объекта.

Удаление лишних объектов в этом уроке не требуется. Главное — понять разницу на будущее.

### Chart исчез после закрытия вкладки

Скорее всего, результат был выполнен через `Run Query`, но сам Chart не был сохранён.

`Run Query` и `Save` — разные действия:

```text
Run Query → выполнить текущую конфигурацию
Save      → сохранить Chart как объект Superset
```
...
- вы умеете открыть сохранённый Chart, изменить параметр отображения и сохранить изменение поверх него.
...
В официальном руководстве Superset создание визуализации начинается из Dataset через `Explore`, изменения применяются через `Run Query`, а сохранённый Chart затем доступен через общий список `Charts`. Для временных данных отдельно используются временная колонка и `Time Grain`.
